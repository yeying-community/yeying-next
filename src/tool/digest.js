import {encodeHex} from './codec.js'
 ...
 */
export async function digest(algorithm, ...messages) {
  const message = messages.reduce((acc, curr) => acc + "" + curr);
  const result = await crypto.subtle.digest(algorithm, new TextEncoder().encode(message))
 ...
 *
 */
export function mergeHash(hashList) {
  if (hashList.length === 0) {
    return undefined
  } else if (hashList.length === 1) {
    return hashList[0]
  } else {
    const hashBuffer = new ArrayBuffer(32 * hashList.length)
    const hashTemp = new Uint8Array(hashBuffer)
    hashList.forEach((e, i) => hashTemp.set(new Uint8Array(e), i * 32))
    return computeHash(hashBuffer)
  }
}
...
function sha256(m, H) {
  let w = [], a, b, c, d, e, f, g, h, i, j, t1, t2
