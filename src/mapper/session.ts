...
export class SessionMapper {
    ...
    constructor(instance :IndexedCache) {
        ...
        const indexes = [this.sessionIdIndexColumn, this.typeIndexColumn, this.formatIndexColumn, this.timestampIndexColumn, this.referenceIdIndexColumn,]
    ...
    cursorBySession(sessionId: string, callback:  (record: any) => void) {
        return this.instance.cursorIndex(this.tableSchema.name, this.sessionIdIndexColumn.keyPath, sessionId, callback, 'next')
    }

    cursorByTimestamp(direction: IDBCursorDirection = 'next', callback:  (record: any) => void) {
        return this.instance.cursorIndex(this.tableSchema.name, this.timestampIndexColumn.keyPath, "", callback, direction)
    }
    ...
    deleteBySessionId(sessionId: string) {
        return new Promise((resolve, reject) => {
            this.instance.deleteByIndex(this.tableSchema.name, this.sessionIdIndexColumn.keyPath, sessionId).then(data => resolve(data), reject)
        })
    }
    ...
    deleteByReferenceId(referenceId: string) {
        ...
        return new Promise((resolve, reject) => {
            this.instance.deleteAllByIndex(this.tableSchema.name, this.referenceIdIndexColumn.keyPath, referenceId).then(data => resolve(data), reject)
        })
    }
