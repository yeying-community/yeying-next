...
export class DownloadProcessor implements Processor {
    ...
    async start(c: CommandMessage): Promise<ProcessMessage> {
        ...
        const success = (a: any) => this.callback({workerId: c.workerId, msgId: c.msgId, processType: 'COMPLETE', payload: a}, message.merged ? [a.data] : undefined)
        const error = (e: any ) => this.callback({workerId: c.workerId, msgId: c.msgId, processType: 'ERROR', payload: e.message})
        const callback = message.merged ? undefined : (b: any, d: any) => this.callback({workerId: c.workerId, msgId: c.msgId, processType: 'DATA', payload: {block: b, data: d}}, [d])
