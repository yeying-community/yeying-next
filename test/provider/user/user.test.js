...
describe('User', () => {
  it('add', async () => {
    ...
    await userProvider.addUser('test1', '13584001111', 'yeying.community@gmail.com')
    console.log(`Success to add new user=${identity.blockAddress.identifier}`)
  })

  it('get', async () => {
    const userProvider = new UserProvider(new Authenticate(identity), provider)
    const user = await userProvider.getUser(identity.blockAddress.identifier)
    console.log(`Success to get user=${JSON.stringify(user)}`)
  })

  it('mod', async () => {
    const userProvider = new UserProvider(new Authenticate(identity), provider)
    await userProvider.modUser('test2')
    console.log(`Success to mod user=${identity.blockAddress.identifier}`)
  })

  it('del', async () => {
    const userProvider = new UserProvider(new Authenticate(identity), provider)
    await userProvider.delUser(identity.blockAddress.identifier)
